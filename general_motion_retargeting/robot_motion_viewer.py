...
class RobotMotionViewer:
    def __init__(self,
        ...
        # 禁用重力，防止 passive viewer 后台 mj_step 让机器人自由落体干扰 qpos
        ...
        def _key_callback(keycode):
            if keycode == 27:  # Esc: 关闭 viewer
                self.viewer.close()
            elif keyboard_callback is not None:
    ...
    def step(self, 
        ...
        self.data.qvel[:] = 0   # 清零速度，防止 viewer 后台 mj_step 通过速度积分漂移 qpos
        mj.mj_forward(self.model, self.data)
