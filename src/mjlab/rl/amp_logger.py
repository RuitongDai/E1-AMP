"""AMP-specific logger matching ATOM's terminal output format."""
...
class AmpLogger(Logger):
  """Logger with dedicated AMP reward buffers and console formatting."""
  ...
  def __init__(
    ...
    self.total_rewbuffer: deque[float] = deque(maxlen=100)
    self.style_rewbuffer: deque[float] = deque(maxlen=100)
    self.cur_total_reward_sum = torch.zeros(num_envs, dtype=torch.float, device=device)
    self.cur_style_reward_sum = torch.zeros(num_envs, dtype=torch.float, device=device)
  ...
  def process_env_step(
    ...
    """Accumulate per-step rewards and flush on episode end."""
    if self.writer is not None:
      ...
      if style_rewards is not None and total_rewards is not None:
        self.total_rewbuffer.extend(
          self.cur_total_reward_sum[new_ids][:, 0].cpu().numpy().tolist()
        )
        self.style_rewbuffer.extend(
          self.cur_style_reward_sum[new_ids][:, 0].cpu().numpy().tolist()
        )
  ...
  def log(
    ...
    """Log metrics to W&B/TB and print to console in ATOM format."""
    ...
    if len(self.rewbuffer) > 0:
      ...
      if len(self.total_rewbuffer) > 0:
        self.writer.add_scalar(
          "AMP/mean_total_reward",
          statistics.mean(self.total_rewbuffer),
          it,
        )
      if len(self.style_rewbuffer) > 0:
        self.writer.add_scalar(
          "AMP/mean_style_reward",
          statistics.mean(self.style_rewbuffer),
          it,
        )
    ...
    # -- Console output --
    log_string = f"""{"#" * width}\n"""
    header = f" Learning iteration {it}/{total_it} ".center(width)
    log_string += f"""\033[1m{header}\033[0m \n\n"""
    ...
    if len(self.rewbuffer) > 0:
      if self.cfg["algorithm"]["rnd_cfg"]:
        log_string += (
          f"""{"Mean extrinsic reward:":>{pad}} """
          f"""{statistics.mean(self.erewbuffer):.2f}\n"""
          f"""{"Mean intrinsic reward:":>{pad}} """
          f"""{statistics.mean(self.irewbuffer):.2f}\n"""
        )
      if len(self.total_rewbuffer) > 0:
        log_string += (
          f"""{"Mean AMP total reward:":>{pad}} """
          f"""{statistics.mean(self.total_rewbuffer):.2f}\n"""
        )
      if len(self.style_rewbuffer) > 0:
        log_string += (
          f"""{"Mean AMP style reward:":>{pad}} """
          f"""{statistics.mean(self.style_rewbuffer):.2f}\n"""
        )
      log_string += (
        f"""{"Mean reward:":>{pad}} """
        f"""{statistics.mean(self.rewbuffer):.2f}\n"""
