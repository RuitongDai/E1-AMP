...
class RslRlPpoAlgorithmCfg:
  ...
  """Symmetry configuration for data augmentation and mirror loss."""
  class_name: str = "PPO"
...
class RslRlBaseRunnerCfg:
  ...
  False to keep metric logging but avoid storage usage. Default is False."""
...
class RslRlAmpDiscriminatorCfg:
  ...
  """Hidden dimensions for the discriminator MLP."""
  activation: str = "relu"
  ...
  """Interpolation weight: 0 = pure style, 1 = pure task reward."""
  lsgan_reward_scale: float = 0.25
  """Scale in LSGAN reward: clamp(1 - scale*(d-1)^2, 0). Larger = sharper."""


@dataclass
class RslRlAmpCfg:
  """Top-level AMP configuration embedded in the algorithm config."""

  loss_type: str = "LSGAN"
  """Discriminator loss type: GAN, LSGAN, or WGAN."""
  disc_obs_buffer_size: int = 500
  """Replay buffer capacity (number of transitions stored)."""
  disc_trunk_weight_decay: float = 1e-4
  """L2 weight decay for discriminator trunk layers."""
  disc_linear_weight_decay: float = 1e-4
  """L2 weight decay for discriminator output layer."""
  grad_penalty_scale: float = 5.0
  """Scale for gradient penalty term."""
  amp_discriminator: RslRlAmpDiscriminatorCfg = field(
...
class RslRlPpoAmpAlgorithmCfg(RslRlPpoAlgorithmCfg):
  """PPO algorithm config with AMP extension fields."""

  amp_cfg: RslRlAmpCfg = field(default_factory=RslRlAmpCfg)
  """AMP-specific configuration."""
  class_name: str = "mjlab.rl.ppo_amp:PPOAMP"
  """Algorithm class name resolved by RSL-RL."""


@dataclass
class RslRlAmpOnPolicyRunnerCfg(RslRlBaseRunnerCfg):
  """On-policy runner config for AMP training."""
  ...
  critic: RslRlModelCfg = field(default_factory=RslRlModelCfg)
  """The critic configuration."""
  algorithm: RslRlPpoAmpAlgorithmCfg = field(default_factory=RslRlPpoAmpAlgorithmCfg)
  """The algorithm configuration (PPO + AMP)."""
