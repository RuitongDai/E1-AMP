...
def register_mjlab_task(
  ...
  Args:
    ...
    env_class: Optional custom env class. If None, uses ManagerBasedRlEnv.
  """
  if task_id in _REGISTRY:
    raise ValueError(f"Task '{task_id}' is already registered")
  _REGISTRY[task_id] = _TaskCfg(env_cfg, play_env_cfg, rl_cfg, runner_cls, env_class)
...
def load_env_class(task_name: str) -> type | None:
  """Load the environment class for a task.

  If None, the default ManagerBasedRlEnv will be used.
  """
  return _REGISTRY[task_name].env_class
