  ...
  def create_gui(
    ...
    with server.gui.add_folder(name.capitalize()):
      ...
      for label, max_val in axes:
        # Play configs can fix a command axis to 0.0 (e.g. no turning), but the
        # GUI "Max" slider requires initial_value to be within [min, max].
        max_gui = max(abs(max_val), 0.1)
        max_input = server.gui.add_slider(
          f"Max {label}",
          initial_value=max_gui,
          step=0.1,
          min=0.1,
          max=10.0,
        )
